...
class CheckInViewModel @Inject constructor(
    ...
    init {
        viewModelScope.launch {
            employeeRepo.activeEmployees.collect { list ->
                _state.update { it.copy(mechanics = list.filter { e -> e.role != EmployeeRole.MANAGER }) }
            }
        }
    }
    ...
    fun submit() {
        ...
        when {
            s.registration.isBlank()  -> { _state.update { it.copy(error = "Registration number is required.") }; return }
            s.makeModel.isBlank()     -> { _state.update { it.copy(error = "Make & model is required.") }; return }
            km == null || km <= 0     -> { _state.update { it.copy(error = "Enter a valid odometer reading.") }; return }
            s.condition == null       -> { _state.update { it.copy(error = "Select the vehicle condition.") }; return }
            s.mechanicId == null      -> { _state.update { it.copy(error = "Assign a lead mechanic.") }; return }
        }
        ...
        viewModelScope.launch {
            try {
                ...
                _state.update { CheckInUiState(mechanics = it.mechanics,
                    success = "Truck ${s.registration} checked in!") }
            } catch (e: Exception) {
