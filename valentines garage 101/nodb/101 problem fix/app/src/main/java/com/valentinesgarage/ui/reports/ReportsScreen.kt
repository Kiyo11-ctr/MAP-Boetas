...
fun ReportsScreen(vm: ReportsViewModel = hiltViewModel()) {
    ...
    Scaffold(topBar = { TopAppBar(title = { Text("Reports", fontWeight = FontWeight.Bold) }) }) { padding ->
                ...
                Column(Modifier.padding(16.dp)) {
                    Row(
                        ...
                        Column {
                            ...
                            Text(r.employee.role.name.replace('_', ' '), fontSize = 12.sp,
                                color = MaterialTheme.colorScheme.onSurfaceVariant)
                        }
                        Surface(
                            color = if (r.completionPercent >= 80) Color(0xFFF0FDF4)
                            ...
                            Text(
                                ...
                                color = if (r.completionPercent >= 80) Color(0xFF16A34A)
            ...
            Card(Modifier.fillMaxWidth(), elevation = CardDefaults.cardElevation(1.dp)) {
                Column(Modifier.padding(16.dp)) {
                    ...
                    Row(horizontalArrangement = Arrangement.spacedBy(20.dp)) {
                        InfoChip("Check-In KM", "${vt.vehicle.odometerAtCheckin} km")
                        InfoChip("Priority",   vt.vehicle.priority.name)
                        InfoChip("Status",     vt.vehicle.status.name.replace('_', ' '))
                    }
                    if (vt.vehicle.conditionNotes.isNotBlank()) {
                        Spacer(Modifier.height(8.dp))
                        Text(
                            ""${vt.vehicle.conditionNotes}"",
                            fontSize = 12.sp,
                            color = MaterialTheme.colorScheme.onSurfaceVariant
                        )
                    }
