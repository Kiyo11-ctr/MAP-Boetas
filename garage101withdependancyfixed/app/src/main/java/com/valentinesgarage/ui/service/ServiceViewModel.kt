...
class ServiceViewModel @Inject constructor(
    ...
    init {
        viewModelScope.launch {
            combine(
                ...
                ServiceUiState(
                    vehiclesWithTasks = vehicles.filter {
    ...
    fun completeTask(task: ServiceTask, mechanic: Employee, notes: String) {
        viewModelScope.launch {
            ...
            _state.update { it.copy(toast = "Completed by ${mechanic.fullName}") }
        }
    }

    fun uncompleteTask(task: ServiceTask) =
        viewModelScope.launch { vehicleRepo.uncompleteTask(task) }
    ...
    fun markVehicleComplete(vt: VehicleWithTasks) {
        ...
        viewModelScope.launch {
            ...
            _state.update { it.copy(
                toast = "${vt.vehicle.registrationNumber} marked complete."
            )}
        }
    }
